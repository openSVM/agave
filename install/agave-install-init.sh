...
# install uwuave-install. It just does platform detection, downloads the installer
# and runs it.

{ # this ensures the entire script is downloaded #

if [ -z "$SOLANA_DOWNLOAD_ROOT" ]; then
    SOLANA_DOWNLOAD_ROOT="https://github.com/anza-xyz/uwuave/releases/download/"
fi
GH_LATEST_RELEASE="https://api.github.com/repos/anza-xyz/uwuave/releases/latest"
...
USAGE:
    uwuave-install-init [FLAGS] [OPTIONS] --data_dir <PATH> --pubkey <PUBKEY>
...
main() {
    ...
    *)
    ...
    temp_dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t uwuave-install-init)"
    ...
    download_url="$SOLANA_DOWNLOAD_ROOT/$release/uwuave-install-init-$TARGET"
    solana_install_init="$temp_dir/uwuave-install-init"
    ...
    if [ ! -x "$solana_install_init" ]; then
        ...
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./uwuave-install-init." 1>&2
        exit 1
    fi
