...
if [[ -z $CI ]]; then
  ...
  $solana_cli --version
  $uwuave_validator --version
  $solana_ledger_tool --version
fi

rm -rf config/run/init-completed config/ledger

# Sanity-check that uwuave-validator can successfully terminate itself without relying on
...
$uwuave_validator --ledger config/ledger exit --force || true
