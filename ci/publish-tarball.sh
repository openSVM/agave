...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/uwuave-install-init uwuave-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml uwuave-install-init-"$TARGET"* $MAYBE_TARBALLS; do
