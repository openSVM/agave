...
FLAGS:
        ...
        --no-prefix           Do not require \`uwuave-\` or \`solana-\` prefix on PACKAGE_NAME
...
if ${require_prefix} && ! [[ "${package_name}" =~ ^(uwuave|solana)- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `uwuave-` or `solana-`' 1>&2
  display_help
  exit 1
fi
...
repository = "https://github.com/anza-xyz/uwuave"
license = "Apache-2.0"
