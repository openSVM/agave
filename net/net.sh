...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip uwuave-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'uwuave-install deploy ...' for the
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying uwuave-install update: $updatePlatform"
    (
      set -x

      scripts/uwuave-install-update-manifest-keypair.sh "$updatePlatform"
